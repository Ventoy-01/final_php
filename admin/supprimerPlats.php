    ...
    <?php
                    ...
                    if ($query->rowCount()>0) {
                      foreach ($resultat as $value) {
                        ...
                        ?>
                        <form action="./TraitementSupprimerPlats.php" method="post" class="contact-form">

                            <input type="hidden" name="codeid" value="<?php echo $_GET['codeid'] ?>" />       

                            <label for="nom">Nom Plat</label>
                            <input type="text" name="nom" id="nom" disabled value="<?php echo $value->nom_plat; ?>" />

                            <label for="cuisson">Cuisson plat</label>
                            <input type="text" name="cuisson" disabled id="cuisson" value="<?php echo $value->cuisson_plat; ?>"/>

                            <label for="prix">Prix</label>
                            <input type="number" name="prix" disabled id="prix" value="<?php echo $value->prix_plat ;?>" />
                            
                            <label for="quantite">Quantite Plats</label>
                            <input type="number" name="quantite" disabled id="quantite" value="<?php echo $value->quantite_plats; ?>"/>
